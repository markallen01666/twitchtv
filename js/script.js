...
$(document).ready(function() {
  for (i = 0; i < twitchArray.length; i++) {
    switch (twitchArray[i]["stream"]) {
      case undefined:
        console.log("Undefined");
        break;
      case null:
        $('#feed' + (i + 1)).html('<a href="' + twitchArray[i]["_links"]["channel"] + '" target="_blank"> <h2><strong>' + twitchArray[i]["display_name"] + ': </strong></h2>Offline</a>');
        break;
      default:
        $('#feed' + (i + 1)).html('<a href="' + twitchArray[i]["_links"]["channel"] + '" target="_blank"><div class="col-xs-2"><img class="img-responsive logo" src="' + twitchArray[i]["stream"]["logo"] + '"></div><div class="col-xs-10"><h2><strong>' + twitchArray[i]["stream"]["display_name"] + '</div><div class="col-xs-2"></div><div class="col-xs-10"></strong></h2>' + twitchArray[i]["stream"]["status"] + '</div></a>');
    }
  }
});
